...
#SBATCH --output=combine_loop8.out 
#SBATCH --error=combine_loop8.err
...
for POP in "${POPS[@]}"; do
    for CONF in "${ET_CONFIGS[@]}"; do
        for FMIN in "${CE_FMINS[@]}"; do
            
            # Construct the detector string
            DETECTORS="CE40km_${CE}:${FMIN} ${CONF}"
            
            echo "Processing -> Pop: ${POP} | ET Config: ${CONF} | CE fmin: ${FMIN} Hz"
            ...
            python combine_results_network_parallel.py \
                ...
                --out_path "/home/lreali1/scr16_berti/luca/stm_cbc_runs/${POP}s/networks/networks8" \
                --snr_th 10.0 \
                --cores 48
