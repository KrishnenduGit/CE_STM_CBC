...
#SBATCH --output=combine_loop7.out 
#SBATCH --error=combine_loop7.err
...
for POP in "${POPS[@]}"; do
    for CONF in "${CE_CONFIGS[@]}"; do
        for FMIN in "${CE_FMINS[@]}"; do
            ...
            DETECTORS="CE40km_${CONF}:${FMIN} CE20km_${CONF}:${FMIN} ${ET_triangle}"
            ...
            python combine_results_network_parallel.py \
                ...
                --out_path "/home/lreali1/scr16_berti/luca/stm_cbc_runs/${POP}s/networks/networks7" \
                --snr_th 10.0 \
                --cores 48
